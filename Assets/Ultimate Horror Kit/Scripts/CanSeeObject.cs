...
namespace AxlPlay
{
    ...
    public class CanSeeObject : MonoBehaviour
    {
        ...
        public Collider[] returnedObjects;
        ...
        public GameObject CanSee()
        {
            ...
            else
            {
                ...
                if (targetObject == null)
                {
                    returnedObjects = MovementUtility.WithinSight(transform, offset, fieldOfViewAngle, viewDistance, objectLayerMask, targetOffset, ignoreLayerMask);
                    bool setActive = false;
                    for (int i = 0; i < returnedObjects.Length; i++)
                    {

                        if (returnedObjects[i].gameObject != this.gameObject && returnedObjects[i].gameObject.tag != IgnoreTag)
                        {
                            returnedObject = returnedObjects[i].gameObject;
                            setActive = true;
                        }
                    }
                    if (!setActive)
